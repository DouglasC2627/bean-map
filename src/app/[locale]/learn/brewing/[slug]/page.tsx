...
export function generateStaticParams() {
  return getArticleSlugs("brewing").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Params): Promise<Metadata> {
  const { locale, slug } = await params;
  const t = await getTranslations({ locale, namespace: "learn" });
  const article = getArticle("brewing", slug, locale);
  if (!article) return { title: t("notFoundTitle") };
  const base = pageMetadata({
    locale,
    path: `/learn/brewing/${slug}`,
...
export default async function BrewingArticlePage({ params }: Params) {
  const { locale, slug } = await params;
    ...
    <article className="mx-auto w-full max-w-(--breakpoint-md) px-4 py-8 pb-24">
      ...
      <JsonLd
        data={breadcrumbSchema(locale, [
          ...
          { name: article.frontmatter.title, path: `/learn/brewing/${slug}` },
        ])}
      />
      <Link
        href="/learn"
